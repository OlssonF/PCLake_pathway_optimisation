...
desired_states <- data.frame(variable = c('oChlaEpi', 'aDFish', "aDSubVeg"),
                             target = c(20, 6, 0.04),
...
obj_function <- function(val_pars, name_pars, future_states) {
  ...
  eval_output <- evaluate_pathway(PCLake_output = model_output, 
                                  ...
                                  eval_target = list(oChlaEpi = function(out,target){(out-target)/target},
                                                     ...
                                                     aDSubVeg = function(out,target){(out-target)/target}))
...
if (make_plots) {
  p4 <- state_pathways |> 
    ...
    geom_line() +
    scale_colour_viridis_c(option = 'A', begin = 0.3, end = 0.9) +
    geom_hline(data = desired_states, aes(yintercept = target), linetype = 'dashed') +
    theme_bw()
  if (save_output) {
