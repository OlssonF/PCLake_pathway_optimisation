...
desired_states_df <- data.frame(opt_var = c('oChlaEpi', 'aDSubVeg'),
                                lower_range = c(0, 50),
                                upper_range = c(20, 100))

desired_states <- list(oChlaEpi = list(target = c(0,20),
                                       weights = 0.5),
                       aDSubVeg = list(target = c(50, 100),
                                       weights = 0.5))
...
obj_function <- function(val_pars, name_pars, future_states) {
  ...
  eval_output <- evaluate_pathway(PCLake_output = model_output, 
                                  ...
                                  eval_target = list("oChlaEpi" = range_obj,
                                                     "aDSubVeg" = range_obj) # see optim_functions.R
...
if (make_plots) {
  p1 <- ggplot(iteration_summary, aes(x=fMarsh_lag, y= fMarsh, size = mPLoadEpi, colour = mPLoadEpi_lag)) +
    geom_point() +
...
if (make_plots) {
  p4 <- state_pathways |> 
    ...
    select(all_of(c('ID', 'year', 'aDSubVeg', 'oChlaEpi', 'mPLoadEpi_use', 'fMarsh_use'))) |> 
    ...
    theme(panel.border = element_rect(colour = 'black'),
          legend.position = 'top',
          panel.spacing.y = unit(c( rep( c( rep(0.2,3), 0.6), 3), rep(0.2,3)),
                                 "lines")
          ) +
