...
## Script purpose: Two horizon example of an optimsation framework using PCLake for a single objective (chlorophyll a < 20 ug/L) and two measures (P load reduction, marsh area) 
...
obj_function <- function(val_pars, name_pars, future_states) {
  ...
  eval_output2 <- evaluate_pathway(PCLake_output = model_output, 
                                   future_states = future_states[[2]],
                                   eval_year = 'max', # evaluate last year
                                   eval_days = 50:300, # try the spring instead
                                   eval_funs = max,
                                   eval_target = list(oChlaEpi = range_obj)) # see optim_functions.R
  ...
  eval_output <- sum(c(eval_output1, eval_output2))
  return(eval_output)
}
